...
inline __device__ void warpScanInclusive2(Metrics& idata,Metrics& idata2, volatile Metrics *s_Data,volatile Metrics *s_Data2, uint size){
...
template <typename Metrics>
inline __device__ void warpScanExclusive2(Metrics& idata,Metrics& idata2, volatile Metrics *s_Data,volatile Metrics *s_Data2, uint size){
...
template <typename Metrics>
inline __device__ void scan1Inclusive2(Metrics& idata,Metrics& idata2, volatile Metrics *s_Data, uint size){

    volatile Metrics* s_Data2;
    s_Data2 = s_Data + blockDim.x*2;
	...
	if(size > WARP_SIZE){
        ...
        if( threadIdx.x < (blockDim.x / WARP_SIZE) ){
            //grab top warp Types
            Metrics val = s_Data[threadIdx.x];
            Metrics val2 = s_Data2[threadIdx.x];
...
inline __device__ void warpCompareInclusive(Metrics& idata,Metrics& idata2, volatile Metrics *s_Data, uint size){
    ...
    s_Data2[pos].idx = 0;
    pos += size;
    s_Data[pos].idx = idata.idx;
    s_Data2[pos].idx = idata2.idx;

    for(uint offset = 1; offset < size; offset <<= 1)
    {
    	s_Data[pos].idx =max(s_Data[pos].idx, s_Data[pos - offset].idx);
    	s_Data2[pos].idx =min(s_Data2[pos].idx, s_Data2[pos - offset].idx);
    }

    idata.idx = s_Data[pos].idx;
    idata2.idx= s_Data2[pos].idx;
}

template <typename Metrics>
inline __device__ void compareInclusive(Metrics& idata,Metrics& idata2, volatile Metrics *s_Data, uint size){
        ...
        if( (threadIdx.x & (WARP_SIZE - 1)) == (WARP_SIZE - 1) )
         {
        	s_Data[threadIdx.x >> LOG2_WARP_SIZE].idx = idata.idx;
        	s_Data2[threadIdx.x >> LOG2_WARP_SIZE].idx = idata2.idx;
         }
        ...
        if( threadIdx.x < (blockDim.x / WARP_SIZE) ){
            //grab top warp Types
        	Metrics val;
            val.idx = s_Data[threadIdx.x].idx;
        	Metrics val2;
            val2.idx = s_Data2[threadIdx.x].idx;
            //calculate exclsive scan and write back to shared memory
             warpCompareInclusive(val,val2, s_Data, size >> LOG2_WARP_SIZE);
             s_Data[threadIdx.x].idx =val.idx;
             s_Data2[threadIdx.x].idx =val2.idx;
        }

        //return updated warp scans with exclusive scan results
        __syncthreads();
        idata.idx=max(idata.idx,s_Data[threadIdx.x >> LOG2_WARP_SIZE].idx) ;
        idata2.idx=min(idata2.idx,s_Data2[threadIdx.x >> LOG2_WARP_SIZE].idx) ;
